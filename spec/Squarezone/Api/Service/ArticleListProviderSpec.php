...
class ArticleListProviderSpec extends ObjectBehavior
{
    ...
    function it_provides_article_list(Request $request, Connection $db)
    {
        ...
        $sql = 'SELECT a.id_article, a.title, a.slug, a.creation_date, ac.slug AS category 
                FROM article a 
                LEFT JOIN article_category ac ON(ac.id_article_category=a.id_article_category) LIMIT 0,25';

        $db->fetchAll($sql)->willReturn(range(1, 15));
    ...
    function it_provides_article_list_for_specific_category(Request $request, Connection $db)
    {
        ...
        $sql = 'SELECT a.id_article, a.title, a.slug, a.creation_date, ac.slug AS category 
                FROM article a 
                LEFT JOIN article_category ac ON(ac.id_article_category=a.id_article_category) WHERE ac.slug="test"';
